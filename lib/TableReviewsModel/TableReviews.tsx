...
export function TableReviews({ services, isLoading, refetchParent }) {
  ...
  const deployMyStrategy = async () => {
    ...
    notifications.show({
      message: `Strategy deployed successfully with address ${strategyAddress}`,
      color: 'green',
    });
  ...
  async function depositFunds() {
    ...
    let txHash = await depositToStrategy(
      signer,
      depositAmount,
      strategyAddress
    );
    notifications.show({
      message: `Deposited successfully with transaction hash ${txHash}`,
      color: 'green',
    });
    setDepositTx(txHash);
  ...
  const rows = services?.map((row) => {
    ...
    return (
      <Table.Tr key={row.id}>
        ...
        <Table.Td>
          <Link href={'#'}>{row.problem_name}</Link>
        </Table.Td>
        <Table.Td>
          <Anchor component="button" fz="sm">
        ...
        <Table.Td>
          {strategyAddress !== '' ? (
            <Badge ml={5} variant="dot" color="teal">
              Deployed
            </Badge>
          ) : (
            <Button
  ...
  return (
    <Table.ScrollContainer minWidth={800}>
      ...
      <Modal
        opened={opened}
        onClose={close}
        title="Deploy veirifer and strategy"
        size={'xl'}
      >
        ...
        <Button
          mb="sm"
          onClick={deployMyStrategy}
          loading={isDeployingonChain}
          disabled={strategyAddress !== ''}
        >
          Deploy strategy
        </Button>
      ...
      <Modal
        opened={opened2}
        onClose={close2}
        title="Deposit funds to the strategy"
        size={'xl'}
      >
        <Button onClick={depositFunds} loading={isDepositing}>
          Deposit
        </Button>
        {depositTx && (
          <>
